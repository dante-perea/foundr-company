...
const baseMarketBySlug: Record<string, MarketInsight> = {
  ...
  'foundr-work': {
    tam: {
      headline: '~$7B (2025) AI agent platforms + frameworks; growing to ~$50B+ by 2030',
      proxy: 'AI agent platforms + agent orchestration frameworks (MarketsandMarkets, Grand View, CB Insights). Adjacent: AI code generation $11.8B (Mordor), AI agent workforce automation $13B (CB Insights enterprise agents).',
      calc: 'AI agents market: $7.84B (2025) → $52.62B (2030) at 46.3% CAGR (MarketsandMarkets). The orchestration / framework slice is roughly 8–12% of that — call it $600M–$900M (2025), growing faster than the platform layer as agents proliferate.',
      sources: [
        s('MarketsandMarkets — AI Agents', 'https://www.marketsandmarkets.com/Market-Reports/ai-agents-market-15761548.html'),
        s('Grand View — AI Agents Market', 'https://www.grandviewresearch.com/industry-analysis/ai-agents-market-report'),
        s('Mordor — AI Code Gen', 'https://www.mordorintelligence.com/industry-reports/ai-code-generation-and-developer-assistant-market'),
      ],
    },
    sam: {
      headline: '~$400M–$800M / yr (2026)',
      proxy: 'AI-native devs + indie/small teams building production agents who need durable execution + state + observability. The framework + managed-runtime wedge.',
      calc: 'LangChain ecosystem at ~2M monthly developers and 100M+ npm/pip downloads; CrewAI 4K signups/week; Mastra ~150K developers in year one. If 5–10% of the ~2M active agent developers adopt a hosted framework runtime at $30–$200/mo, $400–800M addressable.',
      sources: [
        s('LangChain State of AI Agents 2025', 'https://www.langchain.com/stateofaiagents'),
        s('CrewAI traction', 'https://crewai.com'),
        s('Mastra GitHub', 'https://github.com/mastra-ai/mastra'),
      ],
    },
    som: {
      headline: '$25–50M ARR by year 5',
      proxy: 'OSS-first adoption funnel → hosted-runtime conversion. Same playbook as Vercel-for-Next.js or Cloudflare-for-Workers.',
      assumptions: [
        '50–100K monthly OSS users (npm install foundr-work)',
        '8–12% convert to Hosted Solo $29/mo → 4–10K paid, ~$1.5–3.5M ARR',
        '~5% of paid users upgrade to Hosted Pro $199/mo → 200–500 seats, $0.5–1.2M ARR',
        'Scaling to ~10K paid at year 5 with stable conversion = $25–50M ARR',
      ],
      analog: 'LangChain went 0 → $1.1B valuation on OSS-first; LangSmith hosted runtime converts ~5% of free users at $39/seat. Mastra Cloud is following the same OSS-first → hosted-runtime path. Vercel\'s Next.js model is the canonical precedent: free OSS, monetize the runtime.',
      sources: [
        s('LangChain blog', 'https://blog.langchain.dev'),
        s('Mastra blog', 'https://mastra.ai/blog'),
      ],
    },
    incumbents: [
      {
        name: 'LangGraph (LangChain)',
        position: 'Graph-based agent framework; LangChain at ~$1.1B valuation, LangSmith $39/seat hosted',
        url: 'https://www.langchain.com/langgraph',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'Imperative graph code in Python — every node + edge + condition is hand-wired. The "graph" abstraction leaks: developers reinvent state, durability, and resumption per project.' },
          { kind: 'Business model misalignment', detail: 'LangSmith hosted at $39/seat is priced for teams, not for a solo founder running 10 workers at $10/each. The pricing model is the wrong axis (humans, not workers).' },
          { kind: 'Regulatory / channel dependency', detail: 'Heavy reliance on Python ecosystem + LangChain\'s broader API surface — a single LangChain breaking change reverberates through every LangGraph project.' },
          { kind: 'Cultural / incentive trap', detail: '"Build agents with code, not config" is the explicit positioning. A declarative YAML-first framework requires LangChain to walk back the position that earned them their funding round.' },
        ],
      },
      {
        name: 'CrewAI',
        position: '$24.5M raised, 60% of Fortune 500, 4K signups/week, role-playing Python crews',
        url: 'https://crewai.com/pricing',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'Python framework with every agent as a class. No YAML primitive, no CLI-skill model. MCP is bolted on as a tool wrapper, not the substrate.' },
          { kind: 'Business model misalignment', detail: 'Enterprise sales motion ("Request a Demo," not "Sign up"). No indie tier — gap from free OSS → enterprise $99k+/yr leaves solo founders unserved.' },
          { kind: 'Regulatory / channel dependency', detail: 'Insight Partners–led round = enterprise GTM lock-in. Can\'t credibly run a viral OSS / indie loop without channel conflict.' },
          { kind: 'Cultural / incentive trap', detail: '"Role-playing crews" framing demands multi-agent ceremony. Admitting a single declarative worker is enough most of the time undercuts the multi-agent narrative they raised on.' },
        ],
      },
      {
        name: 'Mastra',
        position: 'TypeScript agent framework, ~150K devs year one, Mastra Cloud paid runtime',
        url: 'https://mastra.ai',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'Imperative TypeScript code; every agent is a class + workflow. No declarative spec primitive — Mastra is "LangGraph for TS," same imperative shape.' },
          { kind: 'Business model misalignment', detail: 'Cloud-first runtime priced for production teams; the OSS surface gets less marketing love than the paid hosted. Same Vercel-for-Next.js model but earlier in the curve.' },
          { kind: 'Cultural / incentive trap', detail: '"Code-first agents" is the explicit thesis. A YAML-spec primitive contradicts the "TypeScript developers want code, not config" pitch.' },
        ],
      },
    ],
    winThesis: [
      'Declarative spec, not imperative code. Every incumbent ships a Python or TypeScript framework that makes you write the agent loop. We make the YAML the worker.',
      'Stateful by default. LangGraph requires you to wire checkpointers; CrewAI agents lose state on restart; Mastra leaves persistence to you. Foundr Workers persist across runs, restarts, and model swaps as the default.',
      'OSS-first acquisition, hosted-first monetization. Same playbook that took Vercel from 0 to $9B and LangChain from OSS to $1.1B. The npm install is free; the runtime is paid when it matters.',
      'BYO CLI is the agent-host-portability wedge. Skills work with Claude Code or Codex; the worker is portable. Incumbents tie you to their orchestrator or their Python/TS stack.',
      'Spec-hash versioning. Workers identified by their spec, not by their commit. Reproducible by construction — solves the "framework moved a minor version, my graph broke" complaint that haunts LangGraph + CrewAI.',
      'Founder + indie shape from day one. CrewAI is chasing the Fortune 500; LangChain monetizes per-seat. We price per-worker, not per-human — the gap they can\'t close without burning their motion.',
    ],
  },
