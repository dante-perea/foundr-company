...
const baseMarketBySlug: Record<string, MarketInsight> = {
  'foundr-world': {
    ...
    som: {
      headline: '~$35–60M / yr by 2029',
      proxy: '~5% capture of the AI-solo SAM via the X-tag viral loop + MCP-native distribution.',
      assumptions: [
        '50K paid founders × $70/mo blended ARPU',
        '~70% on Solo $20, ~15% on Pro $95, heavy free-tier drag from X-tag growth loop',
      ],
      analog:
        'Devin scaled $1M → $73M ARR in 9 months on pure self-serve developer motion before any enterprise push (Sacra).',
      sources: [
        s('Sacra — Cognition', 'https://sacra.com/c/cognition/'),
  ...
  'foundr-work': {
    tam: {
      headline: '~$11.8B (2025) → projected $79B by 2031 (37% CAGR)',
      proxy: 'AI code generation & developer-assistant spend (Mordor Intelligence). Upper-bound proxy: ~$250B/yr US software-engineer wage spend that agent-native tools are starting to substitute (per NEA Factory thesis).',
      calc: '$16.13B (2026) growing 37.39% CAGR → $78.97B (2031). Substitution lens: 1% capture of $250B SWE wage spend = $2.5B/yr already redirected to agents.',
      sources: [
        s('Mordor — AI Code Gen Market', 'https://www.mordorintelligence.com/industry-reports/ai-code-generation-and-developer-assistant-market'),
        s('VentureBeat — Anthropic $30B run-rate', 'https://venturebeat.com/technology/anthropic-says-it-hit-a-30-billion-revenue-run-rate-after-crazy-80x-growth'),
        s('NEA — Factory thesis', 'https://www.newenterpriseassociatesnea.com/blog/factory-the-platform-for-agent-native-development'),
      ],
    },
    sam: {
      headline: '~$1.5–2.5B / yr (2026)',
      proxy: 'The slice spent on delegated / background coding agents that ship PRs (not autocomplete IDEs). Solo founders + small teams (≤50), BYO-key tolerant, MCP-native.',
      calc: 'Cursor 1M+ paying individuals (mostly individuals/small teams), Claude Code $2.5B run-rate (~$1B individual/SMB), Replit $253M ARR (non-enterprise), Cognition pre-Windsurf Devin $73M ARR almost entirely self-serve. BYOK adoption doubled 18% → 36% Jan→Apr 2026.',
      sources: [
        s('TheNextWeb — Cursor $2B / 1M devs', 'https://thenextweb.com/news/cursor-anysphere-2-billion-funding-50-billion-valuation-ai-coding'),
        s('SaaStr — Anthropic $14B ARR', 'https://www.saastr.com/anthropic-just-hit-14-billion-in-arr-up-from-1-billion-just-14-months-ago/'),
        s('Sacra — Replit $253M ARR', 'https://sacra.com/research/replit-at-253m-arr-growing-2352-yoy/'),
        s('Ivern — State of AI Agents 2026', 'https://ivern.ai/blog/state-of-ai-agents-developer-survey-2026'),
      ],
    },
    som: {
      headline: '$30–60M ARR by year 5 (~2–4% of the delegated-agent SMB slice)',
      proxy: 'Per-PR transparent pricing + BYO model key wedge.',
      assumptions: [
        '100k Solo seats × $29 × 12 = $35M',
        '5k Pro seats × $199 × 12 = $12M',
        'Blended ARPU ~$45/mo across paid base',
        'Free-tier viral loop (quote-tweet for 5 PRs) is the funnel',
      ],
      analog: 'Devin went $1M → $73M ARR in 9 months on self-serve solo/small-team adoption pre-Windsurf. Replit $10M → $100M in 5.5 months on the indie/non-engineer wedge.',
      sources: [
        s('Sacra — Cognition', 'https://sacra.com/c/cognition/'),
        s('SaaStr — Replit $100M', 'https://www.saastr.com/100mreplit/'),
      ],
    },
    incumbents: [
      {
        name: 'Cursor (Anysphere)',
        position: '$3B ARR (Apr 2026), 1M+ paying users, $50B valuation in talks',
        url: 'https://cursor.com',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'VS Code fork — Electron baggage, monthly upstream rebase tax, IDE chrome IS the product surface (forces synchronous human-in-the-loop). Background Agents bolted on, not the spine.' },
          { kind: 'Business model misalignment', detail: 'Per-seat subscription priced for an IDE session, not for autonomous work-while-you-sleep. Unit of value is keystrokes, not merged PRs.' },
          { kind: 'Regulatory / channel dependency', detail: 'Conditional SpaceX acquisition + xAI Colossus compute tie-up creates model-supply concentration risk; Anthropic-cut precedent (Windsurf) shows labs can throttle competitors.' },
          { kind: 'Cultural / incentive trap', detail: '"Cursor as IDE" identity — Series D blog still pitches "code editor more delightful than the world has ever seen." Hard to pivot to async crew when moat is editor UX.' },
        ],
      },
      {
        name: 'Devin (Cognition)',
        position: '$155M ARR post-Windsurf, $10.2B valuation, raising at ~$25B',
        url: 'https://devin.ai/pricing',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'Single-droid mental model; "Devin" is one agent per task with a confidence meter, not a manager-pattern crew. Windsurf IDE acquired bolted-on doubles the surface area.' },
          { kind: 'Business model misalignment', detail: 'ACU (Agent Compute Unit) pricing is opaque — users can\'t predict cost-per-PR. Enterprise pivot (Goldman/Citi/Dell) is pulling roadmap away from indie founders.' },
          { kind: 'Regulatory / channel dependency', detail: 'Frontier-model passthrough; no BYO-key story — locked to Cognition\'s chosen models and compute.' },
          { kind: 'Cultural / incentive trap', detail: '"Autonomous engineer" framing demands one all-powerful droid; admitting "you need a crew of small specialists with a manager" undercuts the Devin brand narrative.' },
        ],
      },
      {
        name: 'GitHub Copilot (Microsoft)',
        position: '4.7M paid subs, ~$450–850M ARR, 90% of Fortune 100',
        url: 'https://github.com/features/copilot',
        vulnerabilities: [
          { kind: 'Tech debt', detail: 'Carries the entire VS Code + Visual Studio + JetBrains plugin matrix; agent features ship under the IDE-extension org chart, not as a first-class autonomous spine.' },
          { kind: 'Business model misalignment', detail: '$19/$39 per-seat enterprise SKUs assume one human per seat per month; manager-pattern + per-PR pricing is structurally hostile to Microsoft\'s volume-licensing motion.' },
          { kind: 'Regulatory / channel dependency', detail: 'Microsoft–OpenAI co-dependence + EU AI Act + enterprise procurement gravity force conservatism on autonomy.' },
          { kind: 'Cultural / incentive trap', detail: 'GitHub identity is "platform for humans writing code together." Truly autonomous PR-shipping crews threaten the seat-based monetization underneath GitHub Enterprise.' },
        ],
      },
    ],
    winThesis: [
      'Manager-pattern crew, not solo droid — directly attacks Devin/Codex framing. 62% of devs say "keeping track of what each agent is doing" is the biggest blocker (Ivern 2026).',
      'Per-PR transparent pricing eats opaque ACU/seat models. Solo-founder buyers can read a PR-count line item; they can\'t reason about Cognition\'s ACUs.',
      'BYO model key is a wedge incumbents can\'t match without cannibalizing margin. Cursor/Devin/Copilot need model-margin to justify valuations.',
      '10–50 small-repo founder cohort is structurally underserved. Enterprise gravity pulls Cursor, Cognition, GitHub upmarket; the founder-with-15-side-projects is left to a Lovable/Replit single-app loop.',
      'Free quote-tweet onboarding = Replit/Lovable viral funnel. Bypasses seat procurement; PLG analog to Replit $10M → $100M in 5.5 months.',
      'MCP-native crew composes with the foundr.* ecosystem. Incumbents have no equivalent agent-citizenship substrate; the crew is portable across the user\'s whole agentic stack.',
    ],
  },
