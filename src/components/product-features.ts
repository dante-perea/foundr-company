...
export const featuresBySlug: Record<string, FeatureBundle> = {
  'foundr-world': {
    positioning:
      'A persistent isometric world where every founder gets an office and the residents are AI agents that ship while you sleep. Pricing scales with workspaces and MCP calls — never with humans.',
    pricing: [
      {
        name: 'Free',
        price: '$0',
        badge: QT,
        blurb: 'A real office and two agents, on the house.',
        includes: [
          '1 office',
          'Up to 2 concurrent agents',
          '5 furniture slots, public room only',
          '500 MCP tool calls / day',
          '24h activity history',
        ],
        cta: { label: 'Get a free token', href: tweet('I want a foundr.world access token — @perea_ai') },
      },
      {
        name: 'Solo',
        price: '$20 / mo',
        blurb: 'Two workspaces, eight agents, private rooms, a real history.',
        includes: [
          '2 workspaces, up to 8 concurrent agents',
          'Unlimited furniture, public + private rooms',
          '25k MCP tool calls / day',
          '30-day activity history',
          'Custom avatar + custom subdomain',
        ],
        cta: { label: 'Notify me about Solo', href: mailto('foundr.world — Solo waitlist') },
        highlight: true,
      },
      {
        name: 'Pro',
        price: '$95 / mo',
        blurb: 'Multi-workspace, multi-token, team-shareable.',
        includes: [
          '5 workspaces, 25 concurrent agents',
          'Multi-token (5 tokens — team of agents or founders)',
          '250k MCP tool calls / day',
          '1-year activity history',
          'Branded templates, priority MCP lane, room analytics',
        ],
        cta: { label: 'Notify me about Pro', href: mailto('foundr.world — Pro waitlist') },
      },
    ],
    featureGroups: [
      {
    ...
    differentiation: [
      ...
      'Quote-tweet free tier. No card, no waitlist, viral acquisition aligned with distribution.',
      'Solo-founder economy, not per-seat SaaS. Pricing scales with agent compute, never humans.',
  ...
  'foundr-work': {
    positioning:
      'A manager-pattern agent crew that picks tasks off your backlog and ships PRs while you sleep. Per-PR transparent pricing. BYO model key on every tier — you control the spend floor.',
    pricing: [
      {
        name: 'Free',
        price: '$0',
        badge: QT,
        blurb: 'One crew, five PRs, on your own model key.',
        includes: [
          '1 crew (planner + 2 workers)',
          '5 merged PRs / mo',
          'Repos ≤ 5k LOC',
          'BYO Anthropic or OpenAI key required',
        ],
        cta: { label: 'Get a free token', href: tweet('I want a foundr.work access token — @perea_ai') },
      },
      {
        name: 'Solo',
        price: '$29 / mo',
        blurb: 'Three crews, fifty PRs, real repo sizes.',
        includes: [
          '3 crews, 50 PRs / mo',
          'Repos ≤ 50k LOC',
          'BYO key or at-cost token passthrough',
          'Slack + Linear hooks',
          '$0.50 / PR overage',
        ],
        cta: { label: 'Notify me about Solo', href: mailto('foundr.work — Solo waitlist') },
        highlight: true,
      },
      {
        name: 'Pro',
        price: '$199 / mo',
        blurb: 'Unlimited crews, 500 PRs, pooled team spend.',
        includes: [
          'Unlimited crews, 500 PRs / mo',
          'Unlimited repo size',
          '5 team seats ($20 / seat after)',
          'Pooled spend cap, audit log',
          '$0.30 / PR overage, priority worker pool',
        ],
        cta: { label: 'Notify me about Pro', href: mailto('foundr.work — Pro waitlist') },
      },
    ],
    featureGroups: [
      {
        title: 'Core',
        items: [
          'GitHub repo connect with per-repo working agreement (lint, test, review rules)',
          'Planner agent reads agent_backlog and picks the next item',
          'Worker fleet — claim → branch → implement → test → PR',
          'Sandboxed ephemeral runtime per worker (no cross-task contamination)',
          'PR opens with summary, test logs, and "how to verify" block',
          'Self-review pass before requesting human review',
          'Human checkpoint queue — blocks only on decisions that are yours',
          'Cost + PR count dashboard',
        ],
      },
      {
        title: 'Agentic-first (MCP + token)',
        items: [
          'work_assign / work_status / work_review / work_kill / work_brief',
          'OAuth2 bearer + scoped tokens',
          'Any MCP host (Claude Code, Cursor, custom) can drive the crew',
        ],
      },
      {
        title: 'Solo-founder-shaped',
        items: [
          'Fits 10–50 small repos without an enterprise contract',
          'No SSO requirement below Pro',
          'BYO model key on every tier — you control the spend floor',
          'Quote-tweet → free, no credit card',
          'Foundr.* SSO — one identity across the family',
        ],
      },
    ],
    mcp: [
      { name: 'work_assign', desc: '(repo, task_id) — hand a backlog item to the crew' },
      { name: 'work_status', desc: '(crew_id) — what is in flight, what is blocked' },
      { name: 'work_review', desc: '(pr_url) — request the review pass on an external PR' },
      { name: 'work_kill', desc: '(crew_id) — stop a runaway worker' },
      { name: 'work_brief', desc: '(repo) — read or update the working agreement' },
    ],
    differentiation: [
      'Manager-pattern crew, not a solo droid. Planner reads the backlog; N workers execute in parallel.',
      'Backlog-driven loop. Agent picks the next item itself — no per-task prompt.',
      'MCP-first wire format. Same tools callable from any MCP host.',
      'BYO model key as a real tier, not an enterprise checkbox. You control spend.',
      'Per-PR transparent pricing. "23 PRs this month" beats "47 ACUs burned."',
    ],
    competitors: [
      { name: 'Devin (Cognition)', positioning: 'Autonomous SWE agent', pricing: '$20 + $2.25 / ACU', weakness: 'Opaque per-task cost; $400 invoices on a $20 plan', url: 'https://devin.ai/pricing' },
      { name: 'OpenAI Codex Cloud', positioning: 'Cloud + IDE bundled', pricing: '$8–$200', weakness: 'OpenAI models only, no BYO key', url: 'https://chatgpt.com/codex/pricing' },
      { name: 'Cursor Composer / Cloud', positioning: 'IDE-first agent', pricing: '$20–$60 / seat', weakness: 'IDE-shaped, not "work my backlog while I sleep"', url: 'https://cursor.com/pricing' },
      { name: 'OpenHands', positioning: 'Open-source coding agent', pricing: 'Free OSS', weakness: 'You assemble orchestration; no opinionated crew', url: 'https://www.openhands.dev/pricing' },
      { name: 'Factory.ai (Droid)', positioning: 'Agent-native CLI', pricing: '$20–$200', weakness: 'Single droid per session, no crew model', url: 'https://factory.ai/pricing' },
    ],
    cta: {
      headline: 'Send the first crew at your repo',
      body: 'Quote-tweet @perea_ai for a token. Bring your own Anthropic or OpenAI key. The first five PRs are free.',
      primary: { label: 'Get a free token', href: tweet('I want a foundr.work access token — @perea_ai') },
